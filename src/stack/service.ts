...
export default class Service extends cdk.Stack {
  ...
  async initialize() {
    ...
    if (process.env[SERVICE_VAULT_KEY]) {
      const service_vault = await pexec(
        `aws secretsmanager get-secret-value --secret-id ${process.env[SERVICE_VAULT_KEY]} --region "${process.env.AWS_REGION}"`
      );
        ...
        `${this.repo}-${this.key}`,
        {
          ...
          taskImageOptions: {
            ...
            logDriver: ecs.LogDrivers.awsLogs({
              streamPrefix: `${this.repo}-${this.key}`,
            }),
