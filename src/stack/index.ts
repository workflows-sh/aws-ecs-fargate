...
export class Stack{
  constructor(props?: StackProps) {
    ...
    const dev = new Cluster(app, `dev`, {
      repo: repo,
      tag: tag
    }); 
    const stg = new Cluster(app, `stg`, {
      repo: repo,
      tag: tag
    }); 
    const prd = new Cluster(app, `prd`, {
      repo: repo,
      tag: tag
    }); 

    // instantiate a service in dev cluster
    const devService = new Service(app, `dev-${repo}`, {
      repo: repo,
      tag: tag,
      db: dev.db,
      cluster: dev.cluster,
    ...
    const stgService = new Service(app, `stg-${repo}`, {
      repo: repo,
      tag: tag,
      db: stg.db,
      cluster: stg.cluster,
    ...
    const prdService = new Service(app, `prd-${repo}`,{
      repo: repo,
      tag: tag,
      db: prd.db,
      cluster: prd.cluster,
